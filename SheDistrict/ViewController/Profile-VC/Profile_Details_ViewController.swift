...
class Profile_Details_ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if user_idFriend == signUp!.userID {
            btnUserActions.last?.isSelected = true
            for i in 0..<btnUserActions.count {
                if i != 1 {
                    btnUserActions[i].isHidden = true
                }
                if i == 4 {
                    btnUserActions[i].isSelected = true
                    btnUserActions[i].isHidden = false
                }
            }
        }
    ...
    @IBAction func btnUserActions(_ sender:UIButton) {
        if sender.tag == 0 {
            ...
            if sender.isSelected {
                func_Next_VC_Main_2("ProfileViewsBoostsVC")
            } else {
                func_Next_VC_Main_1("UsersAnnouncementVC")
            }
        }
    }
...
// MARK:- FullPictureVC
extension Profile_Details_ViewController:DelegateFullPictureVC {
...
extension Profile_Details_ViewController:DelegateVerificationSuccessfully {
