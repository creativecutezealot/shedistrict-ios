...
class Attending_ViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        arr_select_attend_event.removeAll()
    ...
    @objc func func_get_events() {
        eventsYesterday.removeAll()
        eventsToday.removeAll()
                    ...
                    if let jsonData = json["attending"].description.data(using: .utf8) {
                        do {
                            ...
                            for scheduleEvent in scheduleEvents {
                                let fullDatetime = "\(scheduleEvent.meetingDate ?? "") \(scheduleEvent.meetingTime ?? "")"
                                print(fullDatetime)
                                
                                let dayPostition = fullDatetime.dateDifference().day
                                
                                if fullDatetime.dateDifference().day < 0 || fullDatetime.dateDifference().hour < 0 || fullDatetime.dateDifference().minute < 0 || fullDatetime.dateDifference().second < 0 {
//                                if dayPostition < 0 {
...
extension Attending_ViewController:UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.imgProfileUser.sd_setImage(with: URL(string:(eventSection.userProfile ?? "").userProfile), placeholderImage:k_default_user)
        ...
        if indexPath.section == 0 {
            ...
            let fullDatetime = "\(eventSection.meetingDate ?? "") \(eventSection.meetingTime ?? "")"
            let hourPostition = fullDatetime.dateDifference().hour
            let hours = hourPostition == 1 ? "hour" : "hours"
            cell.lbl_attended.text = "Meeting in \(hourPostition) \(hours)"
        } else {
            let fullDatetime = "\(eventSection.meetingDate ?? "") \(eventSection.meetingTime ?? "")"
            let dayPostition = fullDatetime.dateDifference().day
            let days = dayPostition == 1 ? "day" : "days"
            cell.lbl_attended.text = "Meeting in \(dayPostition) \(days)"
        }
