...
class Create_Your_Profile_VC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if !(signUp?.userProfile.isEmpty)! {
            ...
            img_select_image.sd_setImage(with: URL(string:signUp!.userProfile.userProfile), placeholderImage:k_default_user)
    ...
    override func viewWillAppear(_ animated: Bool) {
        if arr_create_profile_fields.count > 0 {
            ...
            } else if arr_create_profile_fields[1] {//&& arr_create_profile_fields[2] {
...
//                arr_create_profile_fields[2] = true
    ...
    @IBAction func btn_write_bio(_ sender:UIButton) {
        ...
        let writeBioVC = storyboard.instantiateViewController(withIdentifier:"Write_Bio_ViewController") as! Write_Bio_ViewController
        writeBioVC.isEditProfile = self.isEditProfile
        navigationController?.pushViewController(writeBioVC, animated:true)
    }
    ...
    private func func_validation() -> Bool {
        ...
        for i in 0..<view_arr.count {
            if !arr_create_profile_fields[0] {
                view_arr[0].shake()
                view_arr[0].func_error_shadow()
                
                is_valid = false
                break
            } else if arr_create_profile_fields[1] || arr_create_profile_fields[2] || arr_create_profile_fields[3] {
                view_arr[1].func_success_shadow()
                is_valid = true
                
                break
            } else {
                view_arr[1].shake()
                view_arr[1].func_error_shadow()
