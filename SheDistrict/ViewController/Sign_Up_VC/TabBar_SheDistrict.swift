...
var dictNotifRecieveInactive = [AnyHashable:Any]()
...
class TabBar_SheDistrict:UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(funcNotifInactiveEvent), name: NSNotification.Name (rawValue:"notiInactiveEvent"), object: nil)
        NotificationCenter.default.addObserver(self, selector:#selector(showMostThingsPopUp), name: NSNotification.Name (rawValue:"MostCommonThings"), object:nil)
    ...
    override func viewWillAppear(_ animated: Bool) {
        if !dictNotifRecieveInactive.isEmpty {
            DispatchQueue.main.asyncAfter(deadline:.now()+0.2) {
                NotificationCenter.default.post(name: NSNotification.Name (rawValue:"notifRecieveInactive"), object:nil, userInfo:dictNotifRecieveInactive)
            }
            isOnApp = true
        } else {
            isOnApp = true
    ...
    @objc func funcNotifInactiveEvent() {
...
extension TabBar_SheDistrict:Delegate_Report_Resion {
    ...
    @objc func showMostThingsPopUp() {
        if let _ = UserDefaults.standard.object(forKey:"MostCommonThings") {
            
        } else {
            UserDefaults.standard.set(true, forKey:"MostCommonThings")
            let storyboard = UIStoryboard (name: "PopUp", bundle: nil)
            let mostCommonThingsVC = storyboard.instantiateViewController(withIdentifier: "MostCommonThings_VC") as! MostCommonThings_VC
            self.addChild(mostCommonThingsVC)
            
            mostCommonThingsVC.view.transform = CGAffineTransform(scaleX:2, y:2)
            
            self.view.addSubview(mostCommonThingsVC.view)
            UIView.animate(withDuration:0.2, delay: 0, usingSpringWithDamping:0.7, initialSpringVelocity: 0, options: [],  animations: {
                mostCommonThingsVC.view.transform = .identity
            })
        }
    }
