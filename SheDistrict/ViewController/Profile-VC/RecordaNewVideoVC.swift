//  RecordaNewVideoVC.swift
//  SheDistrict
//  Created by appentus on 3/17/20.
...
class RecordaNewVideoVC: UIViewController  {
...
//    @IBOutlet weak var view_record_property:UIView!
    @IBOutlet weak var heightRecorded:NSLayoutConstraint!
...
//        height_record_property.constant = 0
    ...
    override func viewWillAppear(_ animated: Bool) {
        let userIntroVideo = URL(string:(signUp?.userDetails[0].userIntroVideo.userProfile)!)
        
        img_selected.image = userIntroVideo!.createVideoThumbnail()
...
//        userIntroRecord = url_recorded
        ...
        do {
            data_user_intro = try Data (contentsOf:url_recorded)
        } catch {
...
//MARK:- custom camera
extension RecordaNewVideoVC:AVCaptureFileOutputRecordingDelegate {
...
//        previewLayer.isHidden = true
    ...
    private func func_add_video() {
        ...
        heightRecorded.constant = view_recorded.frame.size.width
...
//        videoView.backgroundColor = UIColor .red
    ...
    @IBAction func btn_play (_ sender:UIButton) {
        ...
        if url_recorded == nil {
            let userIntroVideo = URL(string:(signUp?.userDetails[0].userIntroVideo.userProfile)!)
            player = AVPlayer(url:userIntroVideo!)
        } else {
...
//        height_record_property.constant = 145
